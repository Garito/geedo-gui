    ...
    <div class="column">
      <div class="box record">
        <h1 class="title">
          ...
          <span class="pushes-right record">{{ obj.name }}</span>
        </h1>
        <Form name="Record" :schema="$store.getters.schema('Record/update')" :value="value" :errors="$store.state.errors" @submit="data => update(data)" @input="$store.commit('setErrors', null)">
          <div id="Record_layout" class="columns">
            <div class="column">
              <div class="columns">
                <div class="column">
                  <div id="name"></div>
          ...
          <template v-slot:controls>
            <div class="columns">
              <div class="column">
                <button type="submit" class="button is-fullwidth record">{{ $t('Save changes') }}</button>
              </div>
              <div class="column">
...
export default {
  ...
  computed: {
    ...
    value () {
      let obj = this.obj
      let result = {}
      for (let field of Object.keys(this.$store.getters.schema('Record/update').properties)) {
  ...
  methods: {
    async update (data) {
      let opts = { opId: 'Record/update', data: data }
