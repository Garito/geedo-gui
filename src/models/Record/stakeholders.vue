<template>
  <div class="record stakeholders section">
    ...
    <template v-else>
      ...
      <Menu :obj="obj" active="stakeholder" :stats="newsAggrs" />
...
export default {
  name: 'RecordStakeholders',
  components: { Menu, Media, Selector, ListManager, Avatar },
  data: () => ({ opId: 'Record/get_stakeholders', editOwner: null, adding: null }),
  computed: {
  ...
  methods: {
    ...
    async transferOwnership (email) {
      ...
      let result = await this.$store.dispatch('transfer_role', { opId: 'Record/transfer_role', role: 'owner', owner: owner.email, newOwner: email })
      if (result) {
    ...
    async give (user, role) {
      let result = await this.$store.dispatch('give_role', { opId: 'Record/give_role', role: role, email: user })
      if (result) {
        this.adding = false
      }
    },
    withdraw (user, role) {
      this.$store.dispatch('withdraw_role', { opId: 'Record/withdraw_role', role: role, email: user })
    }
  },
